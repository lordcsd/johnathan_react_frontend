        ...
        >
            <div className="flex flex-col justify-between h-full text-slate-700">
                ...
                <div className="flex justify-around">
                    <button
                        onClick={state.promptPurpose === "DELETE" ? deleteAccount : updateAccount}
            ...
            <div className="m-2 sm:m-2 md:m-6 lg:m-10 bg-white p-4 rounded-lg shadow-lg">
                <div>

                    {
                        inputs.map(({ name, placeholder, type }, index) => {
                            type = ['password', 'confirmPassword', 'oldPassword'].includes(name) && !state.showPasswords
                                ? 'password' : type
                            return <input
                                key={index}
                ...
                <div className="flex justify-end">
                    <button
                        onClick={toggleModal}
                        name='UPDATE'
                        className="bg-orange-600 ml-3 text-white p-2 rounded font-bold">Update</button>
